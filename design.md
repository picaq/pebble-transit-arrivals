...
| Behavior | Value | Where |
|---|---|---|
| Max favorites stored | 100 (storage cap — hidden records accumulate until trashed; raised from 20 on 2026-08-06 — the fan-out cost is per agency, not per favorite) | `MAX_FAVORITES`, `index.js` |
| Hide a favorite from the list | (a) farther than `hideFavKm` setting (default 19 km / ~12 mi) — **× `railRadiusX` for a rail favorite** (a `RAIL_AGENCIES` stop or one at a `METRO_HUBS` station, see §9; `stopScale()` gives the per-stop reach and `getFavoriteStatus` returns `far:1`, so rail knowledge stays inside the provider) — reappears when near; (b) `hide` flag (watch unstar or settings toggle). Both keep it saved and cost no payload bytes or API calls while hidden | `buildRows()` + `maxCheckM` arg to `getFavoriteStatus()`, `index.js` / `transit511.js` |
...
| A favorite reached by scrolling **keeps its star** | Every row the phone sends carries the `f` flag, wherever it sits: the page-0 nearby block (`buildRows`) and “Load more” pages (`buildMoreRows`) both star a stop that is a favorite but didn’t make the favorites block — past the hide line, or capped out of it once you have more than `FAV_ROWS_MAX`=10 favorites. Without it, exactly when there are too many favorites to hold in your head, the surplus ones read as ordinary stops and look **unfavorited** (fixed 2026-08-06; the load-more half predates it). Hidden favorites stay unstarred on purpose — hiding says “don’t treat this as a favorite”, and starring the plain row is how you bring it back | `buildRows()`, `buildMoreRows()`, `index.js` |
| “Load more” counts the nearby block, **not the unstarred rows** | `state.moreOff` seeds from everything after the LEADING favorites block, and `buildMoreRows` skips only the favorites actually in that block (nearest `FAV_ROWS_MAX`), not every nearby favorite. The two must agree on which rows page 0 handed over or the next page starts past a stop you never saw — both halves changed with the star above, which put starred rows inside the nearby block for the first time | `setRowsFromResponse()`, `main.js`; `buildMoreRows()`, `index.js` |
| Starring past the storage cap **sheds hidden records first** | `MAX_FAVORITES`=100 is a storage cap over saved records, hidden ones included, and the list is newest-first, so tail-truncating it would silently and permanently delete your OLDEST starred stop once you crossed the cap. Over the cap it drops hidden records (already unfavorited — kept only so re-starring restores the name) oldest-first, and only sheds a live favorite if that isn’t enough, with a log line when it does | `saveFavs()`, `index.js` |
...
| “Nearest” = **effective distance** | Both blocks (favorites and nearby) order by `eff` = real distance ÷ `railRadiusX` for a rail stop, ÷ 1 for everything else (`stopScale()`; the provider returns `eff` beside `dist`, so `index.js` never learns which stops are rail). At 5× a station 3 km out ranks like a 600 m bus stop and interleaves with them; at the 1× default `eff` == `dist` and ordering is exactly as it was. Rows always **display the real distance** — `eff` is a sort key and is never shown. Payload shedding pops the effective-farthest tail, so a scaled-in station is not shed ahead of a bus stop that ranks below it | `stopScale()` + `findNearbyStops()` / `getFavoriteStatus()`, `transit511.js`; `favRows` `_d`, `index.js` |
| What counts as a **rail stop** | Decided **per stop**, not per agency: every stop of `RAIL_AGENCIES` (BA, CT, SB), **plus any stop within `HUB_NEAR_M` = 100 m of one of the 12 `METRO_HUBS`** — Muni Metro’s subway stations (Embarcadero, Montgomery, Powell, Civic Center, Van Ness, Church, Castro, Forest Hill, West Portal, Chinatown–Rose Pak, Union Square/Market, Yerba Buena/Moscone). Those trains are grade-separated and BART-fast while the same lines on the surface are not, so Muni cannot be flagged agency-wide, and 511 publishes nothing that separates them (no `ParentStation`, no mode field, and “Station” in the name also matches surface stops like “Ocean Ave/Balboa Park BART Station”). The hubs are held as **coordinates**, so one proximity test covers both the platforms (within ~50 m of the centroid) and **the stops on the same corner**, which are the same walk once you have decided the train is worth it: 2–7 extra stops per station at 100 m, where 250 m would be 17–24 downtown and hand one intersection most of the list. The test is agency-blind (a Golden Gate stop outside Embarcadero scales too) and is skipped entirely at `railRadiusX` 1 | `METRO_HUBS` / `nearMetroHub()` / `stopScale()`, `transit511.js` |
| Watch list retention | **`LIST_RETAIN_MAX` = 30 retained rows; loading is unlimited.** Down at the bottom appends for as long as stops exist; once the list is full, appending **evicts the oldest non-favorite rows off the top** (`trimRetained()` — favorites stay pinned at the head; selection and the scroll window shift with the rows). Memory is bounded by the cap, not by how far you scroll. Rows you scrolled **far** past are eventually gone if you scroll back up (Up at the top reloads the list from where you are); the cap is sized so a **screen or two** of scrolled-past stops survives, so a fast scroller who overshoots a stop can back up to it (why it went 24 to 30 on 2026-07-17 — right after a load-more the retained rows above the cursor are `30 - favCount - MORE_PAGE - 1`, about 2 screens at the full 10-favorite worst case, `VISIBLE_ROWS` about 5). Pagination uses `state.moreOff`, a monotonic cursor, **not** `rows.length`, so eviction never rewinds it. Was `MAX_LIST_ROWS`=14; **removing the cap entirely on 2026-07-13 crashed real hardware with “memory full”** (playbook §B, seventeenth recurrence — the load-more page parses beside the retained list). Both 24 (from 14) and now 30 are conservative steps, **unmeasured until confirmed on hardware** (chunk headroom on a deep scroll of a full list) — raise it only against the §F instrumentation | `LIST_RETAIN_MAX` + `trimRetained()`, `main.js` |
...
| `radiusM` | 500 | Nearby search radius (× `railRadiusX` for rail stops) |
| `railRadiusX` | 1 (slider 1–30) | How much farther a train or ferry is worth going than a bus. Scales a rail stop’s **reach** (nearby search radius `radiusM` × this; favorites hide line `hideFavKm` × this) and **rank** (real distance ÷ this), for favorites and unstarred stops alike — see §8 and §9. Rail means BART/Caltrain/ferry plus anything at a Muni Metro subway station (§9), which is why the Clay label reads “Train distance multiplier” rather than naming two agencies. Displayed distances stay real. 1 = off |
