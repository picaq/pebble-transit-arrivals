...
approximate (as of 2026-07-07); the constant names are the stable anchors.
...
|---|---|---|
| LIST (`MODE_LIST`) | Header "Transit Glance", then favorites (★, nearest first, dimmed when far/serviceless) followed by nearby stops | App start; Back from ARRIVALS |
...
architecture change, not a style tweak (CLAUDE.md §4).
...
| Text | Where |
|---|---|
| Header title "Transit Glance" | `draw()`, `main.js:197` |
...
| "Now" (arrival due) | `prepareArrivals()`, `main.js:247` |
| Subtitle format `"SF · 320 m · no arrivals"` | built on the **phone**, `buildRows()` in `src/pkjs/index.js:174-191`; distance formatting in `formatDistM()` (meters under 1 km, else `x.y km`) |
...
## 6. Button behavior (`src/embeddedjs/main.js:362-388`)
...
| Select | Open arrivals for highlighted stop | Toggle ★ favorite |
...
`state.arrivalsPending`) — button-mashing without one has crashed the
watch (CLAUDE.md §12 item 12).

## 7. Timing & refresh cadence

| Behavior | Value | Where |
|---|---|---|
| Arrivals auto-refresh while screen open | 60 s | `Timer.repeat` in `openArrivals()`, `main.js:307` |
| Phone-side full-arrivals cache (absorbs manual + auto refresh) | 45 s | `ARRIVALS_TTL_MS`, `transit511.js:318` |
| Favorite has-arrivals cache | 3 min | `ARR_CACHE_TTL_MS`, `transit511.js:232` |
| Stop-list cache | 7 days | `STOP_CACHE_TTL_MS`, `transit511.js:38` |
| Watch request timeout | 15 s | `REQUEST_TIMEOUT_MS`, `protocol.js:44` |
| Phone HTTP timeout | 20 s | `getJSON()`, `transit511.js:45` |
| Geolocation | low accuracy, 10 s timeout, 2 min max age | `handleRequest()`, `index.js:233` |
...
| Dim a favorite | farther than `FAR_M` = 19312 m (12 mi) **or** no arrivals | `index.js:149`; keep in sync with `ARRIVAL_CHECK_MAX_M`, `transit511.js:231` |
| Max favorites | 10 | `MAX_FAVORITES`, `favorites.js:13`; also `parseFavs` cap, `index.js:134` |
...
| Rows payload budget | 700 B — farthest non-favorite stops shed first | `buildRows()`, `index.js:200` |
| Arrivals per stop | max 6 | parse loop, `transit511.js:362` |

## 9. Text truncation lengths

| Text | Cap | Where |
|---|---|---|
| Nearby stop name | 28 chars (16 when >8 stops selected) | `transit511.js:213`, `:194` |
| Favorite name from cache | 24 chars | `transit511.js:266` |
| Arrivals header (stop name) | 18 chars | `shortName()`, `main.js:254-256` |
...
phone-side caps raises payload size — re-check the 700 B budget in §8.

## 10. Settings (Clay — `src/pkjs/config.js`, defaults `index.js:28-33`)
...
## 11. Startup behavior (`main.js:398-408`)

- Before any network: saved favorites render immediately with agency-only
  subtitles, status "Connecting…" (or "Waiting for phone…" if pebblekit
  isn't connected).
