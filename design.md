...
| Starring past the storage cap **sheds hidden records first** | `MAX_FAVORITES`=20 is a storage cap over saved records, hidden ones included, and the list is newest-first, so tail-truncating it silently and permanently deleted your OLDEST starred stop on the 21st star. Over the cap it now drops hidden records (already unfavorited — kept only so re-starring restores the name) oldest-first, and only sheds a live favorite if that isn’t enough, with a log line when it does | `saveFavs()`, `index.js` |
